<?php
	...
	if(!function_exists('add_file_to_upcoming_order')){
		...
		?>
		<div id="wufdc_div">
			<?php echo stripslashes(get_option( 'easy_ufdc_caption' )); ?><br />
			<input type="file" name="file_during_checkout" />
			<small><?php _e("Allowed filesize",'easy-upload-files-during-checkout'); ?>: <?php echo get_option( 'easy_ufdc_max_uploadsize' ); ?></small>        
...
<?php			
			...
			&&
				...
				if($is_checkout || $is_cart){
					...
					if($easy_ufdc_req && !ufdc_file_during_checkout()){
						...
						//pree($ext);pree($doctypes);exit;
						if(!empty($ext) && !in_array($ext, $doctypes)){
							wc_add_notice(sprintf( __( 'The "%s" file type is not allowed.','easy-upload-files-during-checkout'), $ext ), 'error');
							wp_redirect($redir);
							exit;
						}
	...
	if(!function_exists('easy_ufdc_box_order_detail')){
		function easy_ufdc_box_order_detail($post) {
			...
			foreach ( $order->get_items() as $order_item ) {				
				...
				$html_out .= '<strong>';
				$html_out .= sprintf( __('File for product','easy-upload-files-during-checkout').': "%s"', $forproduct);
				$html_out .= '</strong><br>';
				...
				while ($i <= $max_upload_count) {
					...
					$forproduct = get_post_meta( $post->ID, '_woo_ufdc_uploaded_product_name_' . $j, true );
					if( !empty( $url ) && !empty( $name ) ) {
						$html_out .= sprintf( '<a href="%s" target="_blank">%s</a>', $url, $name );
						$upload_count++;
					} else {
						//silence is golden
					}
	...
	function ufdc_plugin_links($links) { 
		...
		if($ufdc_custom){
			...
			$ufdc_premium_link = '<a href="'.$ufdc_premium_link.'" title="' . __('Go Premium','easy-upload-files-during-checkout') . '" target=_blank>' . __("Go Premium",'easy-upload-files-during-checkout') . '</a>'; 
